...
class CompileTimeRunner
{
											...
											?conditionalVariables 	: Expr ) : Expr
	{
		...
		var result = try
		{
			CompileTimeRunner._readFile( fileName, applicationContextName, preprocessingVariables, conditionalVariables, assemblerExpr );
		}
		catch ( err: Dynamic )
		{
			error = err;
			//hex.compiletime.util.ContextBuilder.unregister();
			null;
		}

		var fail = macro function() { hex.unittest.assertion.Assert.fail( "Error caught was: '" + $v{error}+"', It should be: '" + $v{errorMessage} +"'", $v{error}, null ); };
		var success = macro function() { hex.unittest.assertion.Assert.isTrue( true, "", null ); };
		return error == errorMessage ? macro {execute: $success} : macro {execute: $fail};
	}
	...
	{
		var contextName = CompileTimeRunner._getContextName( context );
		return CompileTimeRunner._readFile( fileName, contextName, preprocessingVariables, conditionalVariables, assemblerExpr );
	}
